...
def print_cube(ipaddress,name):
    call('start /wait plink.exe -ssh root@'+ipaddress+' -P 22 -pw vizarchsai123!@# mongod --shardsvr --replSet '+name+' --dbpath /var/lib/mongodb/srs --bind_ip_all --port 27017', shell=True)
...
def crun(ipaddress,file,port):
    call('start /wait plink.exe -ssh root@'+ipaddress+' -P 22 -pw vizarchsai123!@# mongod --configsvr --replSet cf  --dbpath /var/lib/mongodb/'+file+' --bind_ip_all --port '+port,shell=True)
    # print(ipaddress,file,port)
def mongosrun():
    call('start /wait plink.exe -ssh root@10.230.25.98 -P 22 -pw vizarchsai123!@# mongos --configdb cf/10.230.25.98:27017,10.230.25.98:27018,10.230.25.98:27019 --bind_ip_all --port 27020',shell=True)
